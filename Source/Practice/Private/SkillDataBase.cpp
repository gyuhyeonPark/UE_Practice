...
void USkillDataBase::OnPitch_Implementation(APawn* _SkillUser, class APawn* _Target, USkillComponent* _SkillCom)
{
	if (PitchProjectileClass == nullptr || _SkillUser == nullptr)
		return;
	...
	// 스킬 사용자의 스켈레탈 메시 컴포넌트가 있고, FireSock이 존재한다면
	if (_SkillCom->GetMesh() && _SkillCom->GetMesh()->DoesSocketExist(TEXT("PitchSock")))
	{
		FTransform trans = _SkillCom->GetMesh()->GetSocketTransform(TEXT("PitchSock"));
		// FireSock 소켓이 있다면, 해당 소켓의 위치를 투사체 생성 위치로 설정한다.
		InitPos = trans.GetLocation();
	}

	// SkillComponent 에서 설정해둔 투사체를 생성한다.
	if (PitchProjectileClass)
	{
		FActorSpawnParameters SpawnParam = {};
		...
		// 투사체 생성, ProjectileClass(UCLASS 정보)를 토대로 실제 생성 시 원하는 자료형 정보 전달.
/*		APitchProjectile* pProjectile = _SkillUser->GetWorld()->
			SpawnActor<APitchProjectile>(
				PitchProjectileClass,
				InitPos,
				InitRot,
				SpawnParam);*/
		...
		APitchProjectile* pProjectile = _SkillUser->GetWorld()->
			SpawnActorDeferred<APitchProjectile>(
				PitchProjectileClass,
				WorldTrans,
				nullptr,
				nullptr,
		...
		// 투사체에게 사용자와 스킬을 알려준다.
		pProjectile->InitProjectile(_SkillUser, _Target, this);

		pProjectile->FinishSpawning(WorldTrans);
	}
}
